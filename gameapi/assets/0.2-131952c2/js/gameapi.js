		...
		return new Promise(function(resolve, reject) {
			...
			self.firebase.isReady().then(function() {
				M.getRules().then(function() {
					var nbRules = 3;
					var amountThreshold = 50; // Percent
					var amountMinCoverage = 15; // Percent
	...
	gamePrototype.init = function() {
		...
		(function next() {
			...
			if (!window.famobi_gameJS.length) {
				...
				if (self.config.package_id.indexOf('disney-') === 0 && self.config.package_id !== 'disney-makeover-elsa') {
...
fg_api.prototype.debugModule = function() {
	...
	module.prototype.init = function() {
		if (self.getUrlParams()['fg_debug'] || self.debug) {
